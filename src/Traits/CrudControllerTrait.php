...
trait CrudControllerTrait{
    ...
    public function delete(Request $request, $model,$id){
        ...
        if($request->ajax()){ return response()->json("ok",200); }
        else                { return redirect()->back()->with(["message" => "Deleted"]); }
    }
    ...
    public function edit($model, $id){
        ...
        if(method_exists($object,'getEditFormConfig'))  { $formConfig = $object->getEditFormConfig();                       }
        else                                            { $formConfig = include base_path().'/resources/forms/edit/'.$model.'.php'; }
